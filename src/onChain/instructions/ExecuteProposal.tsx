...
export const ExecuteProposal: FC = () => {
  ...
  const onClick = useCallback(async () => {
    try {
      ...
      const ix = await program.methods.executeProposal!()
        .accountsPartial({
      ...
      toast.success(
        "Executed Proposal successfully!! Transaction ID: " + signature,
      );
    } catch (error: any) {
      console.error(error);
    ...
    <div className="flex flex-row justify-center">
      <div className="group relative items-center">
        ...
        >
          ...
          <span className="block group-disabled:hidden">Execute proposal</span>
        </button>
      </div>
    </div>
  );

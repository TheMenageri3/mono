...
export const CreateProposal: FC = () => {
  ...
  const onClick = useCallback(async () => {
    try {
      ...
      const ix = await program.methods.createProposal!(
        id,
        name,
        metadataURI,
      ...
      toast.success(
        "Created Proposal successfully!!! Transaction ID: " + signature,
      );
    } catch (error: any) {
      console.error(error);
    ...
    <div className="flex flex-row justify-center">
      <div className="group relative items-center">
        ...
        >
          ...
          <span className="block group-disabled:hidden">Create proposal</span>
        </button>
      </div>
    </div>
  );

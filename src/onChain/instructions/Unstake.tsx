...
export const UnStake: FC = () => {
  ...
  const onClick = useCallback(async () => {
    try {
      ...
      const ix = await program.methods.unstakeTokens!(staking_amount)
        .accountsPartial({
          mint: mint_dao,
          config,
      ...
      toast.success(
        `UnStaked ${staking_amount.toString()} successfully!! Transaction ID: ` +
          signature,
      );
    } catch (error: any) {
      console.error(error);
    ...
    <div className="flex flex-row justify-center">
      <div className="group relative items-center">
        ...
        >
          ...
          <span className="block group-disabled:hidden">Unstake Tokens</span>
        </button>
      </div>
    </div>
  );

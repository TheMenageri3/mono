...
import type {
  Bounties,
  BountyApplications,
} from "~/server/api/routers/bounty/read";
...
export const mapBountyToTable = (bounties: Bounties[]): tableBounty[] => {
  return bounties.map((bounty) => ({
    ...
    publisher: bounty.company
      ? bounty.company.name
      : (bounty.pointOfContact.username ?? "User"),
    track: bounty.track,
...
export const mapApplicantToTable = (
  applicants: BountyApplications[],
): tableApplicant[] => {
...
export default function Manage({ session }: { session: null | Session }) {
  ...
  const { data: readWorkingBounties, isFetched } =
    api.bounty.readWorkingBounties.useQuery();
  const { data: readOwnedBounties, isLoading } =
    api.bounty.readOwnedBounties.useQuery();
  ...
  return (
    <main className="m-auto my-4 flex w-full flex-1 flex-col px-5 sm:px-12">
      ...
      <div className="flex flex-col">
        {memoizedOwnedBounty.length === 0 && bountyType == "owned" && (
          <H4 className="pt-10 text-center text-zinc-600">
            {!isLoading ? (
              ...
              <span>Loading your bounties ⌛</span>
            )}
          </H4>
        )}
        ...
        {bountyType == "owned" && memoizedOwnedBounty.length > 0 && (
          <Table
            ...
            marginTop="mt-4"
            whenRowClick={
              bountyType == "owned" ? onRowClickOwned : onRowClickWorking
            }
          />
        )}
        {bountyType == "working" && memoizedWorkingBounty.length > 0 && (
          <Table
            ...
            marginTop="mt-4"
            whenRowClick={
              bountyType != "working" ? onRowClickOwned : onRowClickWorking
            }
          />
        )}
      </div>

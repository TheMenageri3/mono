...
export const seed = protectedProcedure.mutation(async ({ ctx }) => {
  ...
  return await db.$transaction(async (tx) => {
    try {
      ...
      const testCompanyContacts = await Promise.all(
      ...
      const createdWorkHistory = await Promise.all(
      ...
      const createdJobApplicationQuestions = await Promise.all(
    ...
    //   });
        ...
        TEST_INTERVIEWS.map((interview) =>
          tx.interview.create({
            data: {
              ...
              jobApplicationId: createdJobApplications[0].id,
              companyContactId: testCompanyContacts[0].id,
            },
          })
        )
      );
        ...
        TEST_PLACEMENTS.map((placement) =>
          tx.placement.create({
            data: {
              ...
              createdById: testUser.id,
              updatedById: testUser.id,
              placementFacilitatorId: TEST_PROFILE.id,
              companyId: testCompany[0].id,
              jobApplicationId: createdJobApplications[0].id,
              profileId: TEST_PROFILE.id,
            },
      ...
      const createdPlacementFeedback = await Promise.all(
      ...
      const createdAssignmentQuestions = await Promise.all(
      ...
      const createdAssignmentSubmissions = await Promise.all(
        TEST_ASSIGNMENT_SUBMISSIONS.map((assignmentSubmission) =>
          tx.assignmentSubmission.create({
            data: {
              ...
              updatedById: testUser.id,
              assignmentId: createdAssignments[0].id,
              submitterId: TEST_PROFILE.id,
              gradedById: TEST_PROFILE.id,
            },
          })
        )
      );
      ...
      const createdAssignmentSubmissionAnswers = await Promise.all(
      ...
      const createdAdminComments = await Promise.all(
      ...
      const createdVenueContactInfo = await Promise.all(
        ...
        TEST_PROJECTS.map((project, index) =>
          tx.project.create({
            data: {
              ...
              createdById: allUsers[index].id,
              updatedById: allUsers[index].id,
      ...
      const createdProjectCollaborations = await Promise.all(
    ...
    //   console.log("Seeding User Skills...");
      ...
      await Promise.all(
        testCompanyContacts.map((contact) =>
          tx.companyContact.update({
            where: { id: contact.id },
            data: {
              roles: {
                connect: createdUserRoles.map((role) => ({ id: role.id })),
              },
            },
          })
        )

...
import {
  deleteClassApplicationResponseSchema,
  restoreClassApplicationResponseSchema,
} from "@/schemas";

const deleteClassApplicationResponse = protectedProcedure
  .input(deleteClassApplicationResponseSchema)
...
const restoreClassApplicationResponse = protectedProcedure
  .input(restoreClassApplicationResponseSchema)
  .mutation(async ({ ctx, input }) => {
    const userId = ctx.session.user.id;
    try {
      const existingClassApplicationResponse =
...
export { deleteClassApplicationResponse, restoreClassApplicationResponse };
